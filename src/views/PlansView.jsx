...
// One accent per tier, low → high, so the eye can tell columns apart at a glance instead of
// reading a flat wall of identical green switches. Falls back to the app's default accent for
// any plan id outside the known Starter→Enterprise ladder (e.g. a newly added plan).
const TIER_ACCENTS = {
  starter: "#64748b",
  growth: "#2563eb",
  advance: "#0891b2",
...
function tierAccent(planId) {
  return TIER_ACCENTS[planId] || "var(--accent)";
}
...
function Toggle({ on, onClick, disabled, size, title, accent }) {
  return (
    <span
      className={`mtx-toggle${on ? " on" : ""}${size === "sm" ? " sm" : ""}${disabled ? " disabled" : ""}`}
      style={accent ? { "--tier": accent } : undefined}
...
export default function PlansView({ plans, modules, labs, onToggleModule, onToggleParam }) {
  ...
  function includedModuleCount(plan) {
    return modules.filter((m) => planIncludesModule(plan, m.key)).length;
  }
        ...
        <table className="plan-matrix">
          <thead>
            <tr>
              ...
              {plans.map((plan) => (
                <th key={plan.id} className="mtx-plan-head" style={{ "--tier": tierAccent(plan.id) }}>
                  <div className="mtx-plan-name">{plan.name}</div>
                  <div className="mtx-plan-count">{includedModuleCount(plan)}/{modules.length} modules</div>
            ...
            {filteredModules.map((mod, idx) => {
              ...
              return (
                <Fragment key={mod.key}>
                  <tr className={`mtx-row${isOpen ? " expanded" : ""}${idx % 2 === 1 ? " alt" : ""}`}>
                    <td
                    ...
                    <tr className="mtx-param-row">
                      <td colSpan={plans.length + 1}>
                        ...
                        >
                          ...
                          {plans.map((plan) => (
                            <div key={plan.id} className="mtx-param-planhead" style={{ "--tier": tierAccent(plan.id) }}>{plan.name}</div>
                          ))}
                          {mod.params.map((p) => (
                            <Fragment key={p.name}>
